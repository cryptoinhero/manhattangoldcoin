...
#if defined(NDEBUG)
# error "Manhattangoldcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xef750371746c101370b108a2fa24e2b01c92af29d2f8430f8a8ec01764655a58");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Manhattangoldcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Manhattangoldcoin Signed Message:\n";
...
{
    // Manhattangoldcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 84000); // Manhattangoldcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.0 * 24 * 60 * 60; // Manhattangoldcoin: 1 days
static const int64 nTargetSpacing = 5.0 * 60; // Manhattangoldcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Manhattangoldcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Manhattangoldcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x57dc0e0ec1e8addc474709fcd3f5f536f3025b30194654c07885d235068b7522");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Glass mountains are easy to see thru";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045e2b264bded66f7e20e58c58643c78b530d86e2915cbc6b0f02b6acb99442c38951ff4664405eb72d93ade4da24ad566adcf16eb8d75870cc4a49736d7c92277") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1539064734;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085029126;

        if (fTestNet)
        {
            block.nTime    = 1539064716;
            block.nNonce   = 385397860;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x9549393f21c0d581706c5e8b8104285a8444f85f7aa464094990dcf6c85a3bf0"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xdd, 0xb4, 0xa2, 0xc1 }; // Manhattangoldcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ManhattangoldcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ManhattangoldcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ManhattangoldcoinMiner(CWallet *pwallet)
{
    printf("ManhattangoldcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("manhattangoldcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ManhattangoldcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ManhattangoldcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ManhattangoldcoinMiner, pwallet));
}
